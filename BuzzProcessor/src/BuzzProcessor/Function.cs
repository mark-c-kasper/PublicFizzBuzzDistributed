...
public class Function
{
    ...
    {
        if (fizzBuzzModel.Number % 5 != 0)
        {
            return;
        }
        
        string currentResponse =
            string.IsNullOrEmpty(fizzBuzzModel.Response) ? string.Empty : fizzBuzzModel.Response;
        fizzBuzzModel.Response = $"{currentResponse}Buzz";
    }
    ...
    private async Task SendMessageToNextQueue(FizzBuzzModel fizzBuzzModel)
    {
        ...
        await sendNumberToSqsProcessor.Send(fizzBuzzModel, fizzBuzzModel.ResultQueue);
    }
}